...
export async function apply_purchase_receipt_stock(
	...
	if (store.has('inventory-cost-entry')) {
		await store.insert('inventory-cost-entry', {
			...
			orden_compra_referencia: text(params.source.referencia_origen),
			producto: params.producto,
			producto_nombre: text(product.name),
			producto_codigo: text(product.codigo),
			proveedor: params.source.proveedor,
			proveedor_nombre: text(params.source.proveedor_nombre),
			proveedor_rfc: text(params.source.proveedor_rfc),
			tipo_origen: text(params.source.tipo_origen) || 'manual',
			cantidad,
...
export async function purchase_order_stats(
	...
	for await (const page of store.scan('purchase-order', {
