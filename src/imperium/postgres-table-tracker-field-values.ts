...
function resolve_descriptor(
	tracker: ImperiumDoc,
	field_path: string,
): TrackerSchemaField | null {
...
function resolve_ref_model(
	...
	resource: string,
	field_path: string,
	descriptor: TrackerSchemaField | null,
) {
...
export async function postgres_table_tracker_field_values(ctx: TrackerFieldValuesCtx) {
	...
	if (!ctx.store.has(TRACKER_RESOURCE)) {
		throw new Error('No se encontró el modelo especificado.');
	}
	const tracker = await lookup_tracker(ctx.store, model_tracker_id);
